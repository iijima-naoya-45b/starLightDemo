...
def create_order(request):
    ...
    try:
        ...
        return JsonResponse({'error': 'Payment API timeout'}, status=504)
```

#### 2. サーキットブレーカーの実装

```python
# ✅ 良い例: pybreakerを使用したサーキットブレーカー
import pybreaker
...
def create_order(request):
    ...
    try:
        result = charge_payment(order.id, order.amount)
...
@shared_task
def charge_payment_task(order_id, amount):
    response = requests.post(
        'https://payment-api.example.com/charge',
        json={'order_id': order_id, 'amount': amount},
        timeout=3
...
def create_order(request):
    ...
    charge_payment_task.delay(order.id, order.amount)
