---
title: "Railsの実行モデルと前提"
label: "Railsの実行モデルと前提"
---
...
3. **シングルスレッド**: GIL（Global Interpreter Lock）により、同時に1つのスレッドのみが実行される（マルチプロセスで並行処理）
4. **メタプログラミング**: 実行時にコードを生成・変更可能
...
- **明示的なトランザクション境界**: `transaction`ブロックで明示
...
- **トランザクション境界**: `transaction`ブロックで明示、after_commitコールバック
