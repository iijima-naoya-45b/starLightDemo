...
依存関係管理とは、**「チームのシナジー（相性）を最大化する」**ことです。React → ReactDOM、Express → Body-Parser——**誰が誰に依存しているか把握しないとチーム崩壊**。例えば、エーフィ（エスパー）がブラッキー（悪）に依存——**進化先が間違うと戦略崩壊**するのと同じです。
...
| 種類 | 比喩 | 実務での用途 | 例 |
|------|--------------|-------------|---|
| **dependencies** | 「バトルメンバー」——本番で取り組む | 本番コードで使用 | `react`, `express`, `axios` |
| **devDependencies** | 「育て屋道具」——育成のみ | 開発・テストのみ | `typescript`, `eslint`, `vitest` |
| **peerDependencies** | 「種類相性」——特定要素と組む | プラグイン・ライブラリ | `react-router` → `react@^18.0.0` |
...
**要素でいえば**:
> 「このプラグインはReact 18以上のチームでしか動かない」——**相性指定**。
...
**要素でいえば**: エーフィがブラッキーに依存、ブラッキーがエーフィに依存——**進化できない**。
...
**要素でいえば**:
> 「チームは6匹まで、全員の相性を把握せよ」——**管理できる範囲で最適を目指す**。
