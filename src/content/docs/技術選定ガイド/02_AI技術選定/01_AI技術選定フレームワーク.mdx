...
AI技術選定とは、**「伝説要素の特性を理解し、最適な1匹を選ぶ」**ことです。GPT-4、Claude、Gemini、ローカルLlama——**それぞれが得意分野を持つ伝説級**。例えば、ミュウツー（汎用最適）、ルギア（安全特化）、レックウザ（マルチモーダル）——**ユースケース次第で最適解が変わる**のがAI選定です。
...
|--------|--------------|--------------|--------------|
| **1. タスク適合性** | 「種類相性」——やりたいことに合っているか | ⭐⭐⭐⭐⭐ | GPT-4で画像生成しようとする（Stable Diffusion使え） |
| **2. 精度** | 「種族値」——出力の正確さ | ⭐⭐⭐⭐⭐ | Llama 2で医療診断→誤診リスク（GPT-4使え） |
| **3. コスト** | 「開発ツール代」——トークン課金 | ⭐⭐⭐⭐ | GPT-4で全リクエスト→月100万円破産 |
...
**ポケモンでいえば**: ストーリー攻略に伝説要素Lv100——**オーバーキル**。
...
### ✅ タスク適合性
- [ ] タスク種類を明確化（text-generation / qa / image-generation等）
...
| 鉄則 | 比喩 | 実務適用 |
|------|--------------|---------|
| **1. タスクに合ったAI** | 種類相性 | テキスト→GPT、画像→Stable Diffusion |
| **2. コスト最適化** | 開発ツール代節約 | 簡単タスクはGPT-3.5 |
...
**ポケモンでいえば**:
> 「伝説要素が最適とは限らない」——**相手（ユースケース）次第で最適解が変わる**。
