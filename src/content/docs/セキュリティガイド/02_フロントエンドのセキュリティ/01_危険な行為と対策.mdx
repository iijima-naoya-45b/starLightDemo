...
フロントエンドのセキュリティは、ユーザーのブラウザで実行されるため、特に注意が必要です。危険な行為を理解し、適切な対策を実施することが重要です。
...
- **公開性**: ソースコードが公開される
- **改ざんの容易さ**: ユーザーがコードを改ざんできる
- **クライアント側の制御**: クライアント側で実行されるため、完全に制御できない

これらの特徴により、フロントエンドでは特別なセキュリティ対策が必要です。

### 危険な行為1: XSS（Cross-Site Scripting）
