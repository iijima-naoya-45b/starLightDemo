...
大きなテーブルを小さなチームションに分割する方法です。

**チームショニングの種類:**
- **範囲チームショニング**: 日付や数値の範囲で分割
- **リストチームショニング**: 特定の値のリストで分割
- **ハッシュチームショニング**: ハッシュ値で分割

**実装例:**

```sql
-- MySQL: 範囲チームショニング（日付で分割）
CREATE TABLE orders (
    id INT PRIMARY KEY,
    user_id INT NOT NULL,
    total DECIMAL(10,2) NOT NULL,
...
-- メリット:
-- 1. クエリが高速（必要なチームションのみスキャン）
-- 2. メンテナンスが容易（古いチームションを削除）
-- 3. バックアップが容易

-- PostgreSQL: 範囲チームショニング
CREATE TABLE orders (
    id INT PRIMARY KEY,
    user_id INT NOT NULL,
    total DECIMAL(10,2) NOT NULL,
